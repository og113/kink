...
for (unsigned int j=0; j<(N+1); j++)
	...
	if (j==0)
		...
		{
		D1.insert(j,j) = 1.0/pow(dr,2.0) + 2.0*(r-h)/pow(r,2.0) - 3.0*pow(y0Vec[j],2.0);
		D1.insert(j,j-1) = -1.0/pow(dr,2.0) - 2.0*(r-h)/pow(r,2.0);
		...
		{
		D1.insert(j,j) = 2.0/pow(dr,2.0) + 2.0*(r-h)/pow(r,2.0) - 3.0*pow(y0Vec[j],2.0);
		D1.insert(j,j+1) = -1.0/pow(dr,2.0);
		D1.insert(j,j-1) = -1.0/pow(dr,2.0) - 2.0*(r-h)/pow(r,2.0);
...
acc(0) = 2.0*(phi(Nt+1) + 2.0*(r-h)(phi(0)-phi(Nt+1))/pow(r,2.0)/dr - phi(0))/pow(dr,2.0) - phi(0) + pow(phi(0),3.0);
...
for (unsigned int j=1; j<N; j++)
    ...
    acc(l) = (phi(l+(Nt+1)) + phi(l-(Nt+1)) - 2.0*phi(l))/pow(dr,2.0) + 2.0*(r-h)(phi(l)-phi(l-(Nt+1)))/pow(r,2.0)/dr - phi(l) + pow(phi(l),3.0);
    acc(l) *= 0.5;
	}
...
for (unsigned int u=1; u<(Nt+1); u++)
    ...
    acc(0) = 2.0*(phi(u+Nt+1) + 2.0*(r-h)(phi(u)-phi(u+Nt+1))/pow(r,2.0)/dr - phi(u))/pow(dr,2.0) - phi(u) + pow(phi(u),3.0);
    for (unsigned int x=1; x<N; x++)
    	{
        unsigned int m = u+x*(Nt+1);
        double r = r0 + x*dr;
        //acc(m) = (phi(m+(Nt+1)) + phi(m-(Nt+1)) - 2.0*phi(m))/pow(dr,2.0) + (phi(m+(Nt+1))-phi(m-(Nt+1)))/r/dr - phi(m) + pow(phi(m),3.0);
        acc(m) = (phi(m+(Nt+1)) + phi(m-(Nt+1)) - 2.0*phi(m))/pow(dr,2.0) + 2.0*(r-h)(phi(m)-phi(m-(Nt+1)))/pow(r,2.0)/dr - phi(m) + pow(phi(m),3.0);
    	}
	}
