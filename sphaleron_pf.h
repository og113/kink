...
int funcPDE (double t, const double y[], double f[], void *params)
	...
	dx /= (double)Nx;
	f[0] = y[1];
	f[1] = sigma*( 2.0*(y[2]-y[0])/pow(dx,2.0) - y[0] + pow(y[0],3.0) );
	for (unsigned int j=1; j<(Nx-1); j++)
		{
		x = x0 + j*dx;
		f[2*j] = y[2*j+1];
		f[2*j+1] = sigma*( (y[2*(j+1)] + y[2*(j-1)] - 2.0*y[j])/pow(dx,2.0) + (y[2*(j+1)]-y[2*(j-1)])/x/dx - y[2*j] + pow(y[2*j],3.0) );
		}
	x = x1 - dx;
	f[2*(Nx-1)] = y[2*(Nx-1)+1];
	f[2*(Nx-1)+1] = sigma*( (y[2*(Nx-2)] - 2.0*y[(Nx-1)])/pow(dx,2.0) -y[2*(Nx-2)]/x/dx - y[2*(Nx-1)] + pow(y[2*(Nx-1)],3.0) );
	return GSL_SUCCESS;
	}
...
int jacPDE (double t, const double y[], double *dfdy, double dfdt[], void *params)
	...
	double dx = x1-x0, x;
	dx /= (double)Nx;
	gsl_matrix_view dfdy_mat = gsl_matrix_view_array (dfdy, Nx, Nx);
	gsl_matrix * m = &dfdy_mat.matrix; 
	gsl_matrix_set_all (m, 0.0);
	for (unsigned int j=0; j<Nx; j++)
		...
		if (j==0)
			...
			gsl_matrix_set (m, 2*j, 2*j+1, 1.0);
			gsl_matrix_set (m, 2*j+1, 2*j, sigma*( -2.0/pow(dx,2.0) - 1.0 + 3.0*pow(y[2*j],2.0) ) );
			gsl_matrix_set (m, 2*j+1, 2*(j+1), sigma*( 1.0/pow(dx,2.0) + 1.0/dx/x ) );
			gsl_matrix_set (m, 2*j+1, 2*(j-1), sigma*( 1.0/pow(dx,2.0) - 1.0/dx/x ) );
			dfdt[2*j] = 0.0;
			dfdt[2*j+1] = 0.0;			
			}
